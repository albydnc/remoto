...
// Function to make a POST request to toggle the pin with timeout
async function togglePin(pin, exp, state) {
    ...
    async function updateStatus() {
      try {
        ...
        Object.keys(data.outputs).forEach(pin => {
          ...
          btn.textContent = data.outputs[pin] ? "Turn OFF" : "Turn ON";
          btn.onclick = () => {
            const newState = data.outputs[pin] ? 0 : 1;
        ...
        // Expansions
        // Update inputs
        const expList = document.getElementById('expansions');
        expList.innerHTML = '';
        Object.keys(data.expansions).forEach(exp => {
          const expTitle = document.createElement('h2');
          expTitle.textContent = "Expansion " + exp;
          expList.appendChild(expTitle);

          const expDiv = document.createElement('div');
          expDiv.className = "status";
          //input list
          const typeLi = document.createElement('h3');
          typeLi.textContent = "Expansion Type: " + data.expansions[exp].type;
          expDiv.appendChild(typeLi);
          //input list
          const inTitle = document.createElement('h2');
          inTitle.textContent = "Inputs:";
          expDiv.appendChild(inTitle);
          const expInList = document.createElement('ul');
          expInList.innerHTML = '';
          Object.keys(data.expansions[exp].inputs).sort((a, b) => {
            ...
            const li = document.createElement('li');
            const led = document.createElement('span');
            // Use the value for the specific pin
            const pinValue = data.expansions[exp].inputs[pin].value;
            led.className = pinValue ? 'led high' : 'led low';
            li.appendChild(document.createTextNode(`${pin} `));
            li.appendChild(led);
            const val = pinValue ? "ON" : "OFF";
            li.appendChild(document.createTextNode(` ${val}  /  ${data.expansions[exp].inputs[pin].volt.toFixed(2)} V`));
            expInList.appendChild(li);
          });
          // Only append if there are children
          if (expInList.childElementCount > 0) {
            expDiv.appendChild(expInList);
          }
          //output list
          const outTitle = document.createElement('h2');
          outTitle.textContent = "Outputs:";
          expDiv.appendChild(outTitle);
          const expOutList = document.createElement('ul');
          expOutList.innerHTML = '';
          Object.keys(data.expansions[exp].outputs).sort((a, b) => {
            const numA = parseInt(a.slice(1), 10);
            const numB = parseInt(b.slice(1), 10);
            return numA - numB;
          }).forEach(pin => {
            ...
            const led = document.createElement('span');
            // Use the value for the specific pin
            const pinValue = data.expansions[exp].outputs[pin];
            led.className = pinValue ? 'led high' : 'led low';
            li.appendChild(document.createTextNode(`${pin} `));
            li.appendChild(led);
            const val = pinValue ? "ON" : "OFF";
            ...
            const btn = document.createElement('button');
            btn.className = 'toggle-btn';
            btn.textContent = data.expansions[exp].outputs[pin] ? "Turn OFF" : "Turn ON";
            btn.onclick = () => {
              const newState = data.expansions[exp].outputs[pin] ? 0 : 1;
          ...
          // Only append if there are children
          if (expOutList.childElementCount > 0) {
            expDiv.appendChild(expOutList);
          }
...
<body>
  ...
  <ul id="expansions" class="status"></ul>
  <div class="config">
    <p>
